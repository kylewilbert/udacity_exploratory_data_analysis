...
knitr::opts_chunk$set(echo=FALSE, warning=FALSE, message=FALSE, fig.width = 6, fig.asp = 0.618)
```
...
```
<br />
This report explores a dataset of about 113,937 loans made by peer-to-peer
lender Prosper between 2005 and 2014.
...
```{r fig.width = 6, fig.asp = 0.618, echo=FALSE}
# view histogram of LoanStatus
summary(loans2$LoanStatus)
qplot(data = loans2, x = LoanStatus) +
  scale_x_discrete(labels = c("Cancelled" = "Cncld", "Chargedoff" = "COff",
                              "Completed" = "Cmpltd", "Current" = "Crnt",
                              "Defaulted" = "Dfltd", 
                              "FinalPaymentInProgress" = "FPIP",
                              "Past Due (>120 days)" = "PD,>120",
                              "Past Due (1-15 days)" = "PD,1-15",
                              "Past Due (16-30 days)" = "PD,16-30",
                              "Past Due (31-60 days)" = "PD,31-60",
                              "Past Due (61-90 days)" = "PD,61-90",
                              "Past Due (91-120 days)" = "PD,91-120")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
# Source: https://www.r-bloggers.com/line-breaks-between-words-in-axis-labels-in-ggplot-in-r/
...
Source: http://archive.fortune.com/2009/04/28/technology/hempel_prosper.fortune/index.htm
...
qplot(x = ProsperRating..Alpha., y= BorrowerRate,
      data = loans2,
      geom = "boxplot") +
  labs(title = "More trustworthy borrowers receive more favorable borrowing rates", 
       x = "Prosper Rating", y = "Borrower Rate (%)")
  
```

### Description One
Post-2009, Prosper tightened up its lending rules and established its own rating
system. The chart above shows clearly that borrowers with better ratings
(AA=best, HR=worst) receive more favorable borrowing rates. We will see this
more clearly in the next plot.

### Plot Two
```{r echo=FALSE, Plot_Two}
# BorrowerRate over time
ggplot(loans2) +
  geom_jitter(aes(x = LoanOriginationQuarter, y = BorrowerRate), alpha = 1/200)+
  labs(title = "Post-2009 Prosper has clearly defined and optimized lending 
       rules compared to pre-2009 Prosper", x = "Loan Origination Quarter", 
       y = "Borrower Rate (%)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
### Description Two
Pre-2009 we can see that many loans had high borrower rates indicating
that these borrowers were quite risky. Post-2009, we see the distribution of
rates begin to change and more loans have lower rates and the rates are more
defined. This indicates that it instituted strong lending rules in order to
establish a more stable portfolio.
...
ggplot(aes(x = LoanOriginationQuarter, y = LenderYield, 
  ...
  scale_color_brewer(type='qual') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Riskier loans yield higher returns for investors",
...
### Description Three
This clearly shows the post-2009 restructuring of lending rules.
In 2009, the yields have a noisy pattern. As time progresses to 2014, 
the pattern becomes much more organized. Riskier loans tend to 
lead to higher yields.
------

# Reflection

I was a bit disappointed in this analysis and found it challenging for a 
number of reasons. I think a subject-matter expert would have been able
to make a more insightful analysis. Since I'm not an expert in p2p lending or
finance, I really struggled to understand what variables from the original 81
I should look into. Even narrowing it down to 16 variables made for a 
challenging exploration. In the end, I simply revealed some of the history
behind the company and helped show why the SEC forced them to reorganize.

With a bit more time to research credit rating, I could possibly show
more insightful analysis showing why pre-2009 Prosper was forced by the SEC
to register.

That being said, I was fairly satisfied with my process once I got the hang of
it. And I can see useful application of this process and these techniques in
my own work at a non-profit.

Sources:
https://en.wikipedia.org/wiki/Charge-off) <br />
https://en.wikipedia.org/wiki/Prosper_Marketplace <br />
https://www.lendacademy.com/prosper-review/ <br />
